...
func init_animations() -> void:
	...
	animation.set_animation("jump", func() -> bool: return velocity.y <= up_direction.y, 4, geral_factor)
	...
	func() -> bool: return is_on_wall_only() && !wall_timer.paused && wall_timer.time_left>0 && current_direction == LookDirection.Right, 
		2, geral_factor)
	animation.set_animation("wall_left", 
	func() -> bool: return is_on_wall_only() && !wall_timer.paused && wall_timer.time_left>0 && current_direction == LookDirection.Left, 
		2, geral_factor)
...
func action_behavior(delta: float) -> void:
	...
	var on_wall: bool = wall_sliding(delta)
	jump_on_tap(on_wall)
	jump_on_hold(on_wall)

func jump_on_tap(on_wall: bool) -> void:
	if Input.is_action_just_pressed("jump"):
		jump_ticks = max_jump_ticks
		if !is_on_floor() && !on_wall && jumps < 2:
			air_jump = true
		jump(jump_force, on_wall)	
		jumps -= 1

func jump_on_hold(on_wall: bool) -> void:
	if Input.is_action_pressed("jump"):
		jump(jump_force, on_wall)	

func jump(power: float, on_wall: bool = false) -> void:
